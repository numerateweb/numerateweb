...
<xsl:template match="om:OMS[@cd='arith1' and @name='plus']"  >
  ...
  <xsl:for-each select="following-sibling::*">
   <xsl:choose>
   <xsl:when test="position() &gt; 1 and self::om:OMI and self::*[translate(.,' ','') &lt; 0]">
    <mo>-</mo>
   </xsl:when>
...
<xsl:template match="om:OMS[(@cd='arith1' or @cd='arith2') and @name='times']"  >
  ...
  <xsl:call-template name="infix">
    ...
    <!-- add operator if RDF expressions are involved as arguments -->
    <xsl:when test="following-sibling::om:OMA[child::om:OMS[@cd='rdf']]">&#xD7;</xsl:when>
    <!-- add operator if arith1.power is involved as an argument -->
    <xsl:when test="following-sibling::om:OMA[child::om:OMS[@cd='arith1' and @name='power']]">&#xD7;</xsl:when>
...
-->
   ...
   <mrow>
   <xsl:apply-templates select="following-sibling::om:OMBIND/om:OMBVAR"/>
   <mspace width="mediummathspace" /><mi>in</mi><mspace width="mediummathspace" /> 
   <xsl:apply-templates select="following-sibling::*[1]"/>
   </mrow>
   </munder>
   </xsl:otherwise>
