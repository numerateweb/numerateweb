...
public abstract class AbstractXMLParser {
	...
	protected String getAttribute(QName name) {
		...
		return reader.getAttributeValue(XMLConstants.NULL_NS_URI
				.equals(namespaceURI) ? null : namespaceURI, name
				.getLocalPart());
	}
	...
	protected ParseException newError(String msg) throws ParseException {
		context.clear();
		return new ParseException(msg, reader.getLocation(),
				reader.hasName() ? reader.getName() : null);
	}
	...
	protected QName nextOrFail(QName... names) throws XMLStreamException,
			ParseException {
	...
	protected String parseStringValue() throws XMLStreamException,
			ParseException {
	...
	private void popContext() throws ParseException {
		if (context.isEmpty()) {
			throw newError("Imbalance between opening and closing tags. Unexpected closing tag: "
					+ reader.getName());
		}
		if (!context.peek().equals(reader.getName())) {
			throw newError("Unexpected closing tag: " + reader.getName()
					+ ". Expected: " + context.peek());
		}
		context.pop();
	}

	protected void required(QName attributeName, Object value)
			throws ParseException {
		if (value == null
				|| (value instanceof String && ((String) value).length() == 0)) {
			throw newError("Attribute " + attributeName
